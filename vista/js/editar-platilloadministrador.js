    ...
    if (form.checkValidity()) {
      switch (formId) {
        ...
        case "formNutriente":
          ...
          pestanaColor(tabId); // Reemplaza 'pestanatab' con el ID de tu pestaña
          // Llamar a la función para actualizar los datos
          actualizarNutrientes(); // Agrega esta línea para llamar a la función de actualización
          break;
...
function actualizarPreparacion() {
    ...
    $('#formPreparacion .formulario-preparacion').each(function(index) {
        ...
        // Obtener el valor del campo idPreparacion
        const idPreparacion = $formularioPreparacion.find('[name=idPreparacion]').val();
        // Enviar -1 si idPreparacion es null o undefined
        formDatos.append('idPreparacion[]', idPreparacion ?? -1);

        // Obtener el valor del campo paso e instruccion
        const paso = $formularioPreparacion.find('[name=paso]').val();
        const instruccion = $formularioPreparacion.find('[name=instruccion]').val();

        // Obtener la imagen seleccionada
        const fotoPreparacionInput = $formularioPreparacion.find('[name=fotoPreparacion]')[0];
        const fotoPreparacion = fotoPreparacionInput.files[0];

        // Crear una variable para almacenar el estado de la foto (-1 si no se sube, 1 si se sube)
        const fotoStatus = fotoPreparacion ? 1 : -1;

        // Enviar el valor de fotoStatus en lugar de la imagen directamente
        formDatos.append('fotoStatus[]', fotoStatus);

        formDatos.append('fotoPreparacion[]', fotoPreparacion);
        formDatos.append('paso[]', paso);
        formDatos.append('instruccion[]', instruccion);
