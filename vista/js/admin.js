...
function enviarSeccion7() {
  ...
  let idPlatillo = idGenerado;
  let seccionDatos=4;
  let seccionIngredientes=4;
  let seccionUtencilio=4;
  let seccionPreparacion=4;
  let seccionNutriente=4;
  let seccionComplemento=4;
...
function registrarTodo() {
  ...
  if (
  !formDatosP.checkValidity() || 
  !formIgredientes.checkValidity() ||
  !formUtencilio.checkValidity() || 
  !formPreparacion.checkValidity()||
  !formComplemento.checkValidity()) {
    // Mostrar mensaje de advertencia utilizando SweetAlert27y6
    Swal.fire({
      icon: 'warning',
